...
class graspingEnv(gym.Env):
    ...
    def reset(self,mode='train',obj_id='4_16'):
    ...
    def step(self, action):
        ...
        info = {'Rotational Form': slip_class, 'Grasping Status': self.grasp_state}
        p.removeBody(self.object)
        # self.step_size *= 0.85
    ...
    def _termination(self):
        if self.step_counter > 100:
            return True, 'Grasping Failure'
        # elif self.obj_current_pose[2] > 0.03 and (not self.slip):
    ...
    def _reward(self,observation):
        ...
        if self.grasp_state == 'Grasping Failure':
            reward = -500
...
def baseline():
    ...
    for test_obj in temp:
        ...
        for i in range(1):
            flag = True
            init_step = 1
            print('共1次探索，第{}次探索'.format(i + 1))
            while True:
                obs = env.reset(mode='test',obj_id=test_obj)  # 020： 100%→100%  991： 100%→100% 604：100%  007 100%
                dir0= -1*obs[0]/abs(obs[0])
                if env.slip:
                    print('初始化成功！')
                    break
                else:
                    ...
                    p.removeBody(env.object)
            for step in range(100):
                actions = [dir0 * init_step]
                print("action", actions)
                obs, reward, done, info = env.step(actions)
                dir1 = -1*obs[0]/abs(obs[0])
                if dir1 != dir0:
                    if flag:
                        init_step *= (5/12*0.5)
                        dir0 = dir1
