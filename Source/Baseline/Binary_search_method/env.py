...
class graspingEnv(gym.Env):
    ...
    def reset(self,mode='train',obj_id='1_17'):
        ...
        print("Initial pose of objects:" + str(self.obj_init_pose))
        self.step_counter = 0
...
def train(mode, arm, render):
    ...
    ppo(env=env,
        ...
        ac_kwargs=dict(hidden_sizes=[4,2]),
        gamma=0.99,
        seed=0,
...
def evaluate():
    ...
    ac = core.MLPActorCritic(env.observation_space, env.action_space, **dict(hidden_sizes=[4,2]))
    checkpoint = torch.load(r".\logs\ppo-ur5-ComGrasp\ppo-ur5-ComGrasp_s0\pyt_save\model.pt")
    ac.load_state_dict(checkpoint['model'])
    print('ac={}'.format(ac))
    #
    temp = ['2_00', '3_01','2_02', '3_03','2_04', '3_05','2_06','3_07','2_08','3_09',
