...
class MLPGaussianActor(Actor):
    ...
    def _distribution(self, obs):
        ...
        mu = self.mu_net(obs)
        print("当前的均值为：", mu)
        self.log_std = torch.nn.Parameter(torch.as_tensor(-3 * np.ones(1, dtype=np.float32)))
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space,
        ...
        if isinstance(action_space, Box):
            self.pi = MLPGaussianActor(obs_dim, action_space.shape[0], hidden_sizes, activation)  # 策略网络
